...
//    private final AbsoluteLayout ProteinScatterPlotContainer;
     ...
     * @param searchingMode
     */
    public PeptidesComparisonsSequenceLayout(final DiseaseGroupsComparisonsProteinLayout cp, int width, DatasetExploringCentralSelectionManager exploringFiltersManagerinst, final boolean searchingMode) {
...
//        int numb = cp.getSignificantDown() + cp.getNotProvided() + cp.getNotReg() + cp.getSignificantUp();
        ...
        GridLayout proteinSequanceComparisonsContainer = new GridLayout(2, cp.getComparison().getDatasetIndexes().length);
        this.addComponent(proteinSequanceComparisonsContainer, 1, 1);
        coverageWidth = (width - 100);
        ...
        int count = 0;
        for (QuantProtein quantProtein : cp.getDsQuantProteinsMap().values()) {
            Label studyTitle = new Label("Study " + (count + 1));
            studyTitle.setStyleName("peptideslayoutlabel");
            proteinSequanceComparisonsContainer.addComponent(studyTitle, 0, count);
            proteinSequanceComparisonsContainer.setComponentAlignment(studyTitle, Alignment.MIDDLE_CENTER);
            ...
            if (dsQuantPepMap.get(quantProtein.getDsKey()) == null) {
                ...
                proteinSequanceComparisonsContainer.addComponent(noPeptidesInfoLabel, 1, count);
                proteinSequanceComparisonsContainer.setComponentAlignment(noPeptidesInfoLabel, Alignment.MIDDLE_CENTER);
                count++;
                continue;
            }
            

            String key = "-" + quantProtein + "-" + cp.getProteinAccssionNumber() + "-";
            PeptidesInformationOverviewLayout peptideInfoLayout = new PeptidesInformationOverviewLayout(cp.getSequance(), dsQuantPepMap.get(quantProtein.getDsKey()), coverageWidth,true);
        ...
        comparisonTitle.setValue(cp.getComparison().getComparisonHeader() + " (#Studies " + count + "/" + cp.getComparison().getDatasetIndexes().length + ")");
...
//            this.generateScatterplotchart(comparisonProtein, coverageWidth, 150);
