 ...
 * @author Yehia Farag
 */
public class IdentificationSearchResultsTableLayout extends VerticalLayout implements Serializable {

    private final IdentificationSearchResultsTable searcheResultsTable;
    private IdentificationSearchResultsTable  currentTable, validatedProteinsTable;
    private final VerticalLayout searchResultsTableLayout = new VerticalLayout();
    private final VerticalLayout exportAllPepLayout = new VerticalLayout();
    private VerticalLayout expBtnProtAllPepTable;
    ...
    public IdentificationSearchResultsTableLayout(CSFPRHandler handler, final Map<Integer, IdentificationDatasetDetailsBean> datasetDetailsList, final Map<Integer, IdentificationProteinBean> fullExpProtList, boolean validatedOnly) {
        ...
        trs1 = new TableResizeSet(currentTable, currentTable.getHeight() + "");//resize tables 
        ...
        VerticalLayout searchingResult = dataExporter.exportSearchingResults(fullExpProtList, true,"Export CSF-PR Search Results" );
        searchingResult.setDescription("Export CSF-PR Search Results");

        exportSearchTableLayout.removeAllComponents();
        exportSearchTableLayout.addComponent(searchingResult);        
        exportSearchTableLayout.setComponentAlignment(searchingResult, Alignment.MIDDLE_LEFT);
    ...
    private final TableResizeSet trs1;
    private final Map<Integer, IdentificationProteinBean> vProteinsList;

    private void selectValidateOnly() {
        searchResultstLabel.setValue("<h4 style='font-family:verdana;color:black;'> Search Results (" + vProteinsList.size() + ")</h4>");
    ...
    private void unselectValidOnly() {
        ...
        searchResultsTableLayout.removeAllComponents();
        validatedProteinsTable = new IdentificationSearchResultsTable(datasetDetailsList, fullExpProtList);
